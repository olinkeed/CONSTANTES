...
class Parser:
    ...
    def statment(self):
        ...
        if tok.tipo == TT_CONST:
            ...
            if self.tokenAtual.tipo != TT_EQ:
                raise ErroSintaxeInvalida("Esperava '=' após nome da constante")
            self.avancar()
            valorNode = self.expr()
            try:
                # 
    ...
    def parse(self):
        ...
        if self.tokenAtual.tipo != TT_EOF:
            raise ErroSintaxeInvalida("Símbolos extras após o fim da expressão", self.tokenAtual.posInicio)
